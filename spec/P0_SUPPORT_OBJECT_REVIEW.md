...
- `app_user`、`audit_log` 和 `system_setting` 的最终字段类型、长度、外键、默认值、索引及事务实现；
- 告警渠道、规则、事件和投递记录；
...
- [ ] 完成 `app_user`、`audit_log` 和 `system_setting` 物理表字典；
- [ ] 继续 Review 告警、外部 API 和 Quartz JDBC JobStore；
