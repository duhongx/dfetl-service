...
## 11. 当前唯一待确认问题：预检“风险展示策略”具体控制什么

当前业务文档已经写入“全局默认 → 数据集覆盖 → 任务覆盖”的预检风险展示策略，但没有定义任何可配置字段或枚举：

- 预检固定由人工启动；
- 未预检、存在问题或结果过期都只展示事实，不阻止任务创建和运行；
- 平台不划分 `WARNING/BLOCKER` 等严重级别；
- 正式执行仍按真实字段和数据合同成功或失败；
- 旧系统实际实现的是“必须有通过的预检”硬门禁，该行为已经被新业务基线明确删除。

因此，当前无法为所谓“预检策略”设计有实际语义的全局、数据集和任务字段。继续保留三级策略会产生没有业务作用的空配置表和 `task_governance_override` 字段。

建议收敛为：

```text
预检事实始终展示
缺失 / PASS / ISSUES / 过期状态始终如实展示
永不作为任务创建或运行门禁
不再建立预检风险展示的全局、数据集和任务策略
```

执行和任务版本只保存需要追溯的事实，例如最近关联的 `precheck_run_id`、是否过期以及源结构/合同 Hash；不保存不存在业务行为差异的“预检策略值”。`task_governance_override` 只承担已经有明确字段的任务级校验覆盖，调度仍进入任务版本，消息仍只在数据集级。

待确认：是否删除“预检风险展示策略”的三级配置，只保留固定的事实展示规则？
