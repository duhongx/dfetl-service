...
> 状态：阶段 1 P0 业务范围与接口语义已确认；物理表字典尚待完成  
> 日期：2026-08-14  
...
接口适配层收到后立即转换为统一的 `targets` 结构。规划、授权、幂等和任务创建只维护一套内部处理逻辑，不为兼容 DTO 复制业务服务。
...
这些内容由 DFETL 根据当前机构、数据集、采集链路和已经确认的业务模型解析。
...
- 任务不存在但机构、数据集或链路条件不满足时返回明确的阻断状态；
...
先完整规划全部原子目标。只要存在以下阻断条件之一，本次不创建任何新任务：
...
需要运行已有任务时，调用独立运行接口。这样可以避免一次“确保任务存在”请求意外触发历史任务。

运行提交发生在任务创建事务提交之后；不同任务的运行提交允许分别成功或失败。

## 8. 响应合同

响应必须按原子目标返回，不能只返回一个模糊的批次结果：
...
| 表 | 职责 |
| --- | --- |
| `external_api_client` | Client 身份、名称、启停、授权模式和密钥密文。 |
...
最终字段类型、密钥生命周期、nonce 保留时间、幂等记录保留时间、删除行为和索引在支撑对象物理表字典复核时完成。
...
- `external_api_client`、授权关联、nonce 和统一幂等请求的物理表字典；
- secret 重置和 client 停用/删除的最终生命周期细节；
- nonce 和幂等请求记录的清理周期；
