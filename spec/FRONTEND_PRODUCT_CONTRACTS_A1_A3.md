...
当前 `web/app/etl` 仍不能作为 A1–A3 验收基线，主要差异如下：

| 当前实现 | 目标合同 |
| --- | --- |
| 存在“业务目录”页面 | 删除业务目录页面，新增业务系统实例 |
| Source 固定归属一家机构和业务目录 | 系统实例与机构、Source 分别多对多 |
| Route 固定单机构，且单独占用主导航 | 采集链路可覆盖多机构，在数据集上下文管理 |
| Task 当前配置可原地编辑、文案称无 Task Version | Task 身份与不可变版本分离 |
| 预检列表以 Run 为顶层 | 列表以 Route 为顶层，Run 为历史事实 |
| 预检详情只展示汇总 | 增加问题记录明细、脱敏、原值查看和导出 |
| RAW 保留固定显示 1 天 | 保留期可配置，具体默认值待 Review |
| Source/Target 分成两个主菜单 | 合并为数据源管理两个页签 |
| 缺少校验策略、Doris 建表、业务系统实例页面 | 按目标信息架构补齐 |
| 账号页面明确“不建设权限” | 权限代码和审计矩阵按本文实施，角色物理模型待 Review |
| 侧栏显示“技术模型已冻结” | 当前阶段是 `IN_PROGRESS`，实施授权按有效 Spec 判断 |
...
| 工作项 | 状态 | 结果 |
| --- | --- | --- |
| A1 数据预检问题明细页面合同 | `CONFIRMED` | 页面层级、明细粒度、筛选、脱敏、导出、到期状态和验收场景已定义 |
| A2 无主键机构范围替换前端语义 | `CONFIRMED` | 用户文案、只读合同、运行确认、状态反馈和审计已定义 |
| A3 页面—操作—权限—审计矩阵 | `CONFIRMED` | 目标信息架构、权限代码、确认等级、审计事件和页面矩阵已定义 |
| 前端代码实施 | `PENDING` | 下一工作包按本文整改路由、菜单、页面、Mock 模型和全部操作 |
| API 合同和物理模型 | `IN_REVIEW` | 在前端产品行为稳定后生成，不由本文直接创建 |
