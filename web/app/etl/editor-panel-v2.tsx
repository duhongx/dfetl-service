...
function baseValues(request: EditorRequest, props: Props): EditorValues {
  ...
  switch (request.kind) {
    ...
    case "source": {
      ...
      return { code: row?.code ?? "", name: row?.name ?? "", institutionCode: row?.institutionCode ?? props.institutions.find((item) => item.status === "ENABLED")?.code ?? "", businessCatalogCode: row?.businessCatalogCode ?? props.catalogs.find((item) => item.status === "ENABLED")?.code ?? "", dbType: row?.dbType ?? "POSTGRESQL", connectionMode: row?.connectionMode ?? "HOST_PORT", host: row?.host ?? "", port: row?.port ?? "5432", database: row?.database ?? "", defaultSchema: row?.defaultSchema ?? "", jdbcUrl: row?.jdbcUrl ?? "", username: row?.username ?? "", password: "", sslEnabled: row?.sslEnabled ?? false, readOnly: row?.readOnly ?? true, queryTimeoutSeconds: String(row?.queryTimeoutSeconds ?? 60), connectTimeoutSeconds: String(row?.connectTimeoutSeconds ?? 10), socketTimeoutSeconds: String(row?.socketTimeoutSeconds ?? 60), poolMaxSize: String(row?.poolMaxSize ?? 4), status: row?.status ?? "ENABLED", description: row?.description ?? "" };
    }
    case "target": {
      const row = editing ? props.targets.find((item) => item.code === request.id) : undefined;
      const endpoints = row?.endpoints.length ? row.endpoints : [{ host: "", queryPort: "9030", httpPort: "8030", enabled: true }];
      return { code: row?.code ?? "", name: row?.name ?? "", database: row?.database ?? "df_ygt", username: row?.username ?? "df_load", password: "", status: row?.status ?? "ENABLED", description: row?.description ?? "", feHosts: endpoints.map((endpoint) => endpoint.host), feQueryPorts: endpoints.map((endpoint) => endpoint.queryPort), feHttpPorts: endpoints.map((endpoint) => endpoint.httpPort), feEnabled: endpoints.map((endpoint) => endpoint.enabled ? "true" : "false") };
    }
    case "route": {
...
function EditorForm(props: Props & { request: EditorRequest }) {
  ...
  const feEnabled = list(values,"feEnabled");

  const addFe = () => setValues((current) => ({ ...current, feHosts:[...list(current,"feHosts"),""], feQueryPorts:[...list(current,"feQueryPorts"),"9030"], feHttpPorts:[...list(current,"feHttpPorts"),"8030"], feEnabled:[...list(current,"feEnabled"),"true"] }));
  const removeFe = (index: number) => setValues((current) => ({ ...current, feHosts:list(current,"feHosts").filter((_,i) => i !== index), feQueryPorts:list(current,"feQueryPorts").filter((_,i) => i !== index), feHttpPorts:list(current,"feHttpPorts").filter((_,i) => i !== index), feEnabled:list(current,"feEnabled").filter((_,i) => i !== index) }));

  const validate = () => {
    const required: Record<EditorKind,string[]> = {
      institution:["code","name"], catalog:["code","name"], source:["code","name","institutionCode","businessCatalogCode","dbType","connectionMode","username","queryTimeoutSeconds","connectTimeoutSeconds","socketTimeoutSeconds","poolMaxSize"], target:["code","name","database","username"], route:["institutionCode","datasetCode","sourceCode","object","targetCode"], task:["name","institutionCode","datasetCode","routeId","scheduleMode"], account:["username","displayName","password","confirmPassword"], profile:["username","displayName"], alertRule:["name","scopeType","metricCode","conditionOp","conditionValue","severity"], alertChannel:["name","channelType","messageFormat","endpoint"], externalClient:["clientId","clientName","authorizationMode"], mapping:["profileName","profileVersion","ruleCode","sourceDbType","sourceTypePattern","recommendedDorisType","compatibilityLevel"],
    };
    ...
    if (request.kind === "source") {
      ...
      if (["queryTimeoutSeconds","connectTimeoutSeconds","socketTimeoutSeconds","poolMaxSize"].some((key) => !integerInRange(text(values,key),1,2147483647))) return "Query/Connect/Socket Timeout 和 Pool Max Size 必须是正整数。";
    }
    if (request.kind === "target") {
      if (!feHosts.length || feHosts.some((host) => !host.trim())) return "Target 至少需要一个完整 FE Endpoint，Host 不能为空。";
      if (feQueryPorts.length !== feHosts.length || feHttpPorts.length !== feHosts.length || feEnabled.length !== feHosts.length) return "FE Endpoint 配置不完整。";
