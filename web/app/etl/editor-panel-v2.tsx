...
function baseValues(request: EditorRequest, props: Props): EditorValues {
  ...
  switch (request.kind) {
    ...
    case "target": {
      ...
      return { code: row?.code ?? "", name: row?.name ?? "", database: row?.database ?? "df_ygt", username: row?.username ?? "df_load", password: "", status: row?.status ?? "ENABLED", description: row?.description ?? "", fe1Host: row?.endpoints[0]?.host ?? "", fe1QueryPort: row?.endpoints[0]?.queryPort ?? "9030", fe1HttpPort: row?.endpoints[0]?.httpPort ?? "8030", fe2Host: row?.endpoints[1]?.host ?? "", fe2QueryPort: row?.endpoints[1]?.queryPort ?? "9030", fe2HttpPort: row?.endpoints[1]?.httpPort ?? "8030" };
    }
    case "route": {
...
function EditorForm(props: Props & { request: EditorRequest }) {
  ...
  return <div className="modal-mask" role="presentation" onMouseDown={props.onClose}><form className="modal editor-modal" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}><header><div><h2>{request.kind === "profile" ? "个人中心" : `${editing ? "编辑" : "新增"}${titles[request.kind]}`}</h2><p>{request.kind === "profile" ? "只维护当前登录账号自身资料和修改密码。" : editing ? "稳定身份字段保持只读；保存修改当前配置。" : "按已冻结产品模型创建当前配置。"}</p></div><button type="button" onClick={props.onClose}>×</button></header><div className="modal-body">
    ...
    {request.kind === "source" && <><div className="editor-grid"><Field label="Source Code" required hint={editing ? "创建后不可修改" : "稳定 Code"}><input value={text(values,"code")} disabled={editing} onChange={(event) => set("code",event.target.value.toUpperCase())}/></Field><Field label="名称" required>{input("name")}</Field><Field label="唯一归属机构" required hint={sourceReferenced ? "已被 Route 引用，归属机构不可修改；需要变更请新建 Source" : undefined}>{select("institutionCode",pairs(props.institutions.filter((item) => item.status === "ENABLED" || item.code === institutionCode),(item) => [item.code,`${item.name} (${item.code})`]),sourceReferenced)}</Field><Field label="业务目录" required hint={sourceReferenced ? "已被 Route 引用，业务分类不可修改" : undefined}>{select("businessCatalogCode",pairs(props.catalogs.filter((item) => item.status === "ENABLED" || item.code === text(values,"businessCatalogCode")),(item) => [item.code,`${item.code} · ${item.name}`]),sourceReferenced)}</Field><Field label="数据库类型" required>{select("dbType",[["POSTGRESQL","PostgreSQL"],["MYSQL","MySQL"],["ORACLE","Oracle"],["SQLSERVER","SQL Server"]])}</Field><Field label="连接模式" required>{select("connectionMode",[["HOST_PORT","HOST_PORT"],["JDBC_URL","JDBC_URL"]])}</Field></div>{text(values,"connectionMode") === "HOST_PORT" ? <div className="editor-grid"><Field label="Host" required>{input("host")}</Field><Field label="Port" required>{input("port")}</Field><Field label="Database" required>{input("database")}</Field><Field label="Default Schema">{input("defaultSchema")}</Field></div> : <Field label="JDBC URL" required>{input("jdbcUrl","jdbc:...")}</Field>}<div className="editor-grid"><Field label="Username" required>{input("username")}</Field><Field label="Password" hint={editing ? "留空表示不修改；不会回显明文" : "保存后只返回掩码"}><input type="password" value={text(values,"password")} onChange={(event) => set("password",event.target.value)}/></Field><Field label="状态">{select("status",[["ENABLED","ENABLED"],["DISABLED","DISABLED"]])}</Field></div></>}
    {request.kind === "target" && <><div className="editor-grid"><Field label="Target Code" required><input value={text(values,"code")} disabled={editing} onChange={(event) => set("code",event.target.value.toUpperCase())}/></Field><Field label="名称" required>{input("name")}</Field><Field label="Database" required>{input("database")}</Field><Field label="Username" required>{input("username")}</Field><Field label="Password" hint={editing ? "留空表示不修改" : "不会回显明文"}><input type="password" value={text(values,"password")} onChange={(event) => set("password",event.target.value)}/></Field><Field label="状态">{select("status",[["ENABLED","ENABLED"],["DISABLED","DISABLED"]])}</Field></div><h3 className="editor-subtitle">FE Endpoints</h3><div className="editor-grid"><Field label="FE #1 Host" required>{input("fe1Host")}</Field><Field label="Query Port">{input("fe1QueryPort")}</Field><Field label="HTTP Port">{input("fe1HttpPort")}</Field><Field label="FE #2 Host">{input("fe2Host")}</Field><Field label="Query Port">{input("fe2QueryPort")}</Field><Field label="HTTP Port">{input("fe2HttpPort")}</Field></div><Field label="说明">{input("description")}</Field></>}
    {request.kind === "route" && <><div className="editor-grid"><Field label="机构" required hint={editing ? "Route 业务身份固定" : "先选机构"}>{select("institutionCode",pairs(props.institutions.filter((item) => item.status === "ENABLED" || item.code === institutionCode),(item) => [item.code,`${item.name} (${item.code})`]),editing)}</Field><Field label="标准 Dataset" required hint={editing ? "Route 业务身份固定" : "一机构 + Dataset 只允许一条未删除 Route"}>{select("datasetCode",pairs(props.datasets.filter((item) => item.status === "ACTIVE" || item.code === datasetCode),(item) => [item.code,`${item.name} · ${item.code}`]),editing)}</Field><Field label="Source" required>{select("sourceCode",[["","请选择 Source"],...pairs(routeSources,(item) => [item.code,item.name])])}</Field><Readonly label="业务目录" value={selectedSource ? `${selectedSource.businessCatalogCode} · ${selectedSource.businessCatalogName}` : "由 Source 自动带出"} hint="只读，不在 Route 重复维护"/><Field label="Schema">{input("schema")}</Field><Field label="Table / View" required>{input("object")}</Field><Field label="对象类型">{select("objectType",[["TABLE","TABLE"],["VIEW","VIEW"],["MATERIALIZED_VIEW","MATERIALIZED_VIEW"]])}</Field><Field label="Target" required>{select("targetCode",pairs(props.targets.filter((item) => item.status === "ENABLED" || item.code === text(values,"targetCode")),(item) => [item.code,item.name]))}</Field><Field label="业务状态">{select("status",[["DISABLED","DISABLED"],["ENABLED","ENABLED"]])}</Field></div><div className="editor-note">保存后按 contract hash 创建或复用不可变 Route Version，并将当前结构状态标记为需要重新核对；业务状态与结构状态不合并。</div></>}
    {request.kind === "task" && <><div className="editor-grid"><Field label="任务名称" required>{input("name")}</Field><Field label="机构" required hint={editing ? "Task 固定身份，编辑不可修改" : "固定身份一部分"}>{select("institutionCode",pairs(props.institutions.filter((item) => item.status === "ENABLED" || item.code === institutionCode),(item) => [item.code,`${item.name} (${item.code})`]),editing)}</Field><Field label="Dataset" required hint={editing ? "Task 固定身份，编辑不可修改" : "固定身份一部分"}>{select("datasetCode",pairs(props.datasets.filter((item) => item.status === "ACTIVE" || item.code === datasetCode),(item) => [item.code,`${item.name} · ${item.code}`]),editing)}</Field><Field label="可用 Route" required hint="只显示同机构+Dataset 且 ENABLED+PASSED 的 Route">{select("routeId",[["","请选择 Route"],...pairs(usableRoutes,(item) => [item.id,`${item.id} · V${item.version} · ${item.sourceName}`])])}</Field></div><h3 className="editor-subtitle">由 Dataset 合同确定</h3><div className="editor-grid"><Readonly label="Task Kind" value={taskContract.taskKind}/><Readonly label="Write Mode" value={taskContract.writeMode}/><Readonly label="Doris Key Model" value={taskContract.keyModel}/><Readonly label="增量字段" value={taskContract.incrementalField}/></div><h3 className="editor-subtitle">读取与调度</h3><div className="editor-grid"><Field label="Fetch Size">{input("fetchSize")}</Field><Field label="Upper Bound Delay (min)">{input("upperBoundDelayMinutes")}</Field><Field label="Lookback (sec)">{input("lookbackSeconds")}</Field><Field label="Schedule Mode">{select("scheduleMode",[["MANUAL","MANUAL"],["EVERY_N_HOURS","EVERY_N_HOURS"],["CRON","CRON"]])}</Field>{text(values,"scheduleMode") === "EVERY_N_HOURS" && <Field label="间隔小时" required>{input("scheduleIntervalHours")}</Field>}{text(values,"scheduleMode") === "CRON" && <Field label="Cron" required>{input("scheduleCron")}</Field>}{text(values,"scheduleMode") !== "MANUAL" && <Field label="Timezone">{input("scheduleTimezone")}</Field>}<Field label="Validation Override">{select("validationOverride",[["INHERIT","INHERIT"],["ROW_COUNT","ROW_COUNT"],["ROW_COUNT_CHECKSUM","ROW_COUNT_CHECKSUM"]])}</Field></div>{checkbox("scheduleEnabled","启用调度（MANUAL 与开关保持正交）")}<div className="editor-note">EVERY_N_HOURS 的最终错峰 Cron 固化到 Task；Dataset Default 只作为创建输入，不热更新已有 Task。</div></>}
