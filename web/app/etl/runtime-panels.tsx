...
export function ExecutionDetailPanel({ executionId }: { executionId: string }) {
  ...
  const sourceEndpoint = snapshot?.sourceRuntime.connectionMode === "JDBC_URL"
    ? snapshot.sourceRuntime.jdbcUrl ?? "—"
    : snapshot ? `${snapshot.sourceRuntime.host}:${snapshot.sourceRuntime.port ?? "—"}` : "—";
  ...
  return <>
    ...
    {snapshot && <>
      <Card title="本次实际 Task 执行参数" note="这些字段是 Execution 接受时实际采用的可变 Task 配置，不从当前 Task 重新推导。"><div className="details"><div><span>Task Kind</span><strong>{snapshot.taskKind}</strong></div><div><span>Write / Key Model</span><strong>{snapshot.writeMode} / {snapshot.keyModel}</strong></div><div><span>Incremental Field</span><strong>{snapshot.incrementalField ?? "无"}</strong></div><div><span>Fetch Size</span><strong>{snapshot.fetchSize}</strong></div><div><span>Upper Bound Delay</span><strong>{snapshot.upperBoundDelayMinutes} min</strong></div><div><span>Lookback</span><strong>{snapshot.lookbackSeconds} sec</strong></div></div></Card>
      <div className="grid-2">
        <Card title="Source Runtime Snapshot" note="只保存/展示非 Secret 运行连接事实。"><div className="details"><div><span>Datasource / Revision</span><strong>{snapshot.sourceRuntime.datasourceId} / r{snapshot.sourceRuntime.revision}</strong></div><div><span>DB / Connection Mode</span><strong>{snapshot.sourceRuntime.dbType} / {snapshot.sourceRuntime.connectionMode}</strong></div><div><span>Endpoint</span><strong>{sourceEndpoint}</strong></div><div><span>Database / Username</span><strong>{snapshot.sourceRuntime.database || "—"} / {snapshot.sourceRuntime.username}</strong></div><div><span>SSL / Read Only</span><strong>{snapshot.sourceRuntime.sslEnabled ? "SSL" : "NO_SSL"} / {snapshot.sourceRuntime.readOnly ? "READ_ONLY" : "READ_WRITE"}</strong></div><div><span>Timeouts</span><strong>Q {snapshot.sourceRuntime.queryTimeoutSeconds}s · C {snapshot.sourceRuntime.connectTimeoutSeconds}s · S {snapshot.sourceRuntime.socketTimeoutSeconds}s</strong></div></div></Card>
        <Card title="Target Runtime Snapshot" note="FE 是本次 Execution 接受时实际选定的非 Secret Runtime Endpoint。"><div className="details"><div><span>Target / Revision</span><strong>{snapshot.targetRuntime.datasourceId} / r{snapshot.targetRuntime.revision}</strong></div><div><span>Database / Username</span><strong>{snapshot.targetRuntime.database} / {snapshot.targetRuntime.username}</strong></div><div><span>FE Endpoints</span><strong>{snapshot.targetRuntime.feEndpoints.map((endpoint) => `#${endpoint.ordinalNo} ${endpoint.host}:${endpoint.queryPort}/http:${endpoint.httpPort}`).join("；")}</strong></div></div></Card>
      </div>
      <Card title="Dataset Message Policy Snapshot" note="Execution 成功后是否创建 Outbox 依据本次快照，不回读当前 Dataset Policy。"><div className="details"><div><span>Enabled / Revision</span><strong>{snapshot.messagePolicy.enabled ? "ENABLED" : "DISABLED"} / r{snapshot.messagePolicy.revision}</strong></div><div><span>Source / Tenant</span><strong>{snapshot.messagePolicy.sourceSystem} / {snapshot.messagePolicy.tenantId}</strong></div><div><span>Routing Key</span><strong>{snapshot.messagePolicy.routingKey || "—"}</strong></div><div><span>Topic</span><strong>{snapshot.messagePolicy.topic || "—"}</strong></div><div><span>Key Template</span><strong>{snapshot.messagePolicy.keyTemplate}</strong></div><div><span>Rate / Page Size</span><strong>{snapshot.messagePolicy.rateLimitPerSecond}/s · {snapshot.messagePolicy.pageSize}</strong></div></div></Card>
    </>}
